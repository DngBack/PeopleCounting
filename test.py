...
class FramesThreadBody:
    # Hàm này dùng để lấy các frame từ một đối tượng camera và đưa vào một hàng đợi
    def __init__(self, capture, max_queue_length=2):
        # Hàm __init__ khởi tạo đối tượng với tham số là đối tượng camera capture, và độ dài tối đa của hàng đợi max_queue_length.
        self.process = True
...
def Tracking(args, config, frame_buffer, bbox_buffer, process_stat):
    ...
    object_recognizer = VectorCNN(
        ie_feat, args.m_reid, args.device2, args.cpu_extension
    )
    ...
    if args.video_streaming:
        ...
        frame_size, fps = (
            config["visualization_config"]["max_window_size"],
            config["visualization_config"]["out_fps"],
        )
        video_streaming = cv2.VideoWriter(
            gst_str_rtp, cv2.CAP_GSTREAMER, 0, fps, frame_size, True
        )
    else:
    ...
    while True:
        ...
        fps = round(1.0 / latency)

        vis = visualize_multicam_detections(
            frames, tracked_objects, fps, **config["visualization_config"]
        )
        draw_detection_range(
            vis,
            config["mct_config"]["w_skip_ratio"],
            config["mct_config"]["h_skip_ratio"],
        )
        if not args.no_show:
        ...
        print(
            f"\rfps = {fps} (avg_fps = {round(1. / avg_latency.get_avg(), 1)})", end=""
        )
...
def main():
    ...
    parser.add_argument(
        "--video_streaming",
        action="store_true",
        help="Stream output video to server using GStreamer",
    )
    parser.add_argument(
        "--device1",
        type=str,
        default="CPU",
        help="Device for detection model inference",
    )
    parser.add_argument(
        "--device2", type=str, default="CPU", help="Device for feature model inference"
    )
    parser.add_argument("--broker_url", type=str, required=True, help="MQTT Broker url")
    parser.add_argument(
        "--method", type=str, default="mtmct", help="Object tracking method"
    )
    parser.add_argument("--cam_id", type=int, default=1, help="Camera ID")
    parser.add_argument(
        "--initial_id", type=int, default=1, help="Initial ID of the first track"
    )
    parser.add_argument(
        "-l",
        "--cpu_extension",
    ...
    detection = mp.Process(
        target=Detection,
        args=(args, config, frame_buffer, bbox_buffer, process_stat),
        daemon=True,
    )
    tracking = mp.Process(
        target=Tracking,
