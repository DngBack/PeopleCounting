...
# Để giao tiếp với MQTT, một giao thức truyền thông giữa các máy chủ và máy khách.
...
class MultiCameraTracker:
    def __init__(
        ...
        self.mqtt_client = Messages(
            f"lab314up{cam_id}", f"lab314/up{cam_id}", broker_url=broker_url
        )

    def process(self, frame, all_detections):
        all_detections, screen_edge_objects, boundary_coord = self._filter_detections(
            all_detections[0], frame.shape
        )
    ...
    def _merge_all(self):
        ...
        for i, sct_track in enumerate(self.sct.tracks):
            for j, mct_track in enumerate(self.tracks):
                if (
                    sct_track.cam_id == mct_track.cam_id
                    and sct_track.id == mct_track.id
                ):
                    dist = self._get_global_clusters_distance(
                        self.tracks[j], self.sct.tracks[i]
                    )
        ...
        distance_matrix = np.zeros(
            (len(mct_track_indices), len(stable_sct_track_indices)), dtype=np.float32
        )

        for i, mct_track_idx in enumerate(mct_track_indices):
            for j, sct_track_idx in enumerate(stable_sct_track_indices):
                distance_matrix[i, j] = self._get_global_rectification_distance(
                    self.tracks[mct_track_idx], self.sct.tracks[sct_track_idx]
                )
        ...
        while distance_matrix.shape[0] > 0 and distance_matrix.shape[1] > 0:
            ...
            if dist < self.global_match_thresh:
                self.sct.tracks[stable_sct_track_indices[j]].global_merge(
                    self.tracks[mct_track_indices[i]], dist
                )
        ...
        for trajectory in self.trajectories:
            for i, track in enumerate(self.sct.tracks):
                if track.id == trajectory[1] and track.cam_id == trajectory[2]:
                    if (
                        not track.off
                        and track.trajectory == self.sct.cam_id
    ...
    def _transmit_info(self):
        for i, track in enumerate(self.sct.tracks):
            if (
                track.feats_delivery_status
                and track.off
                and track.f_avg.is_valid()
                and len(track.f_clust) >= 1
            ):
                self.mqtt_client.deliver_feats(
                    track.id,
                    track.cam_id,
                    track.f_avg.get_avg(),
                ...
                self.trajectories = list(
                    filter(lambda x: x is not None, self.trajectories)
                )

            if track.pos_delivery_status and not track.off and track.cross_camera_track:
                self.mqtt_client.deliver_pos(
                    track.id, track.cam_id, self.sct.cam_id, track.last_merge_dis
                )
    ...
    def _receeive_info(self):
        ...
        self.mqtt_client.pos_temp = list(
            filter(lambda x: x is not None, self.mqtt_client.pos_temp)
        )
        ...
        self.mqtt_client.feats_temp = list(
            filter(lambda x: x is not None, self.mqtt_client.feats_temp)
        )
    ...
    def check_processed_frame_num(self):
        ...
        while hit_times < 2:
            ...
            self.mqtt_client.frame_num_temp = list(
                filter(lambda x: x is not None, self.mqtt_client.frame_num_temp)
            )
    ...
    def _filter_detections(self, detections, frame_shape):
        ...
        for det in detections:
            if (
                det[0] >= 0
                and det[1] >= 0
                and det[2] < frame_shape[1]
                and det[3] < frame_shape[0]
            ):
                w = det[2] - det[0]
                h = det[3] - det[1]
                ar = h / w
    ...
    def _get_global_rectification_distance(self, track1, track2):
        if (
            track1.f_clust.shape[0] > 0
            and len(track2.f_clust) > 0
            and track1.f_avg.shape[0] > 0
            and track2.f_avg.is_valid()
        ):
            f_complex_dist = cosine_distance(
                track1.f_clust, track2.f_clust.get_clusters_matrix()
            )
...
class Messages:
    def __init__(self, clientname, topic, broker_url="127.0.0.1", broker_port=1883):
        self.pos_temp = []
        self.feats_temp = []
        self.frame_num_temp = []
        ...
        self.client = mqtt.Client(
            client_id=clientname, protocol=mqtt.MQTTv311, transport="tcp"
        )  # creating a client instance
        self.client.on_connect = self.on_connect
        self.client.on_message = self.on_message
        self.client.on_subscribe = self.on_subscribe
        self.client.connect(
            broker_url, port=broker_port, keepalive=60
